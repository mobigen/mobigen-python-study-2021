...
def run_command(data: Union[Tuple[str, str, str], Tuple[str, str]]) -> str:
    ...
    if command == 'get':
        if key in server_data_buffer:  # 키가 있는 경우
            msg = f"Data of [{key}]: {server_data_buffer[key]}"
        else:
            msg = f"Fail to get value of [{key}]."
    elif command == 'put':
        value = data[2]
        if key in server_data_buffer:  # 키가 있는 경우
            msg = f"Fail to put [{key}-{value}] pair." \
                   f" Because there is already [{key}] in server. Use another key."
        else:
            server_data_buffer[key] = value
            msg = f"Success to put [{key}-{value}] pair."
    else:  # 아직 지원하지 않는 명령어
...
def main():
    ...
    while True:
        ...
        read_socket_list, _, exception_socket_list = select.select(socket_list, [], socket_list, timeout)
        ...
        for readable_socket in read_socket_list:
            ...
            if readable_socket == server_socket:  # client 가 server 로 접속한 것
                ...
                try:
                    ...
                    data = receive_data(sock=readable_socket)
                ...
                # 명령어 결과 클라이언트로 전송
                send_data(sock=readable_socket, data=result_data)

        # exception 이 발생한 socket 이 있는 경우
        for exception_socket in exception_socket_list:
            logger.warning(f"exception occur: client: {exception_socket}")
            exception_socket.close()
            socket_list.remove(exception_socket)
